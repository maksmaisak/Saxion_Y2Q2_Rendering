...
namespace en {
    ...
    struct ResourceLoader<TResource, std::enable_if_t<isCallable<decltype(&TResource::load), filename_t>>> {

        inline static std::shared_ptr<TResource> load(filename_t filename) {

            if constexpr (std::is_convertible_v<std::result_of_t<decltype(&TResource::load)(filename_t)>, std::shared_ptr<TResource>>)
